...
function SocialIcon({
  ...
  return (
    <a
      ...
      aria-label={label}
      className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#4A4A4A] text-white transition-colors hover:bg-[#3d3d3d] sm:h-10 sm:w-10"
    >
      {children}
    </a>
  )
...
export default function FooterSocialLinks({ links }: { links: SocialLink[] }) {
  const active = links.filter((link) => link.enabled && link.href.trim())
